...
public class ComprasDAO {
    ...
    public int guardarDetalleCompra(DetalleCompra dc) {
        String sql = "insert into Detalle_Compras(idReporte,idCompras, Cantidad, PrecioCompra)values(?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, dc.getIdreporte());
    ...
    public List Detalle(int id) {
        List lista = new ArrayList();
        String sql = "select DC.idDetalle, P.Nombres, DC.idCompras, DC.Cantidad, DC.PrecioCompra FROM detalle_compras DC inner join reporte P on P.idReporte = DC.idReporte where idCompras=" + id;
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                dcom.setReporte(new Reporte());
                dcom.getReporte().setNombres(rs.getString(3));
